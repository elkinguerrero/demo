...
@section('body')
    <div class="row" style="width:99%">
        <div class="col-5 offset-3" style="text-align: center; margin-top: 10%;background-color:#007bff1c; border-radius: 10px; padding: 1% 4%;">
            <form action="{{ route('editarusuario', $usuarios_id->id)}}" method="POST">
                @method('PUT')
                @csrf
                <h2 style="margin: 20px 0 20px 0; color:#0d6efd">Editar Usuario</h2>
                <input type="text" name="name" class="form-control " placeholder="Nombre" style="margin: 20px 0 0 0;" value="{{$usuarios_id->name}}">
                <input type="text" name="correo" class="form-control " placeholder="Correo" style="margin: 20px 0 0 0;" value="{{$usuarios_id->email}}">
                ...
                <select class="form-control" name="perfil" style="margin: 20px 0 0 0;">
                    <option value="1" @if ($usuarios_id->perfil == 1)
                        {{ "selected" }}
                    @endif>Administrador</option>
                    <option value="0" @if ($usuarios_id->perfil == 0)
                        {{ "selected" }}
